...
def GestisciLogin():
    context_type = request.headers.get('Context-Type')
    if context_type == 'application/json':
        jsonReq = request.json
        sUsernameInseritoDalCliente = jsonReq["username"]  
        if sUsernameInseritoDalCliente in utenti:
            sPasswordInseritaDalCliente = jsonReq["password"]
            if sPasswordInseritaDalCliente == utenti[sUsernameInseritoDalCliente]["password"]:
                sPriv = utenti[sUsernameInseritoDalCliente]["privilegi"]
                return jsonify({"username": sUsernameInseritoDalCliente, "privilegi": sPriv})
            else:
                return  jsonify({"Esito": "000", "Msg": "Credenziali errate"})
        else:
            return  jsonify({"Esito": "001", "Msg": "Utente non trovato"})
    else:
        return  jsonify({"Esito": "002", "Msg": "Formato richiesta non supportato"})
