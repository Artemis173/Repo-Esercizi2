...
(define (AddKV json pbuf actionl auxiliary-info)
  ...
  (ifnot (ConnectToTheBlockchain)
	 ...
	 (receive (authorized username) (CheckJwtToken pbuf cfg::FUNUF::secret "AddKV")
	   (Show! "In AddKV: " authorized ", " username)
	   (ifnot authorized
		  (JSON-ANSWER::error "Not authorized")
		  (let* ((dummy (Show! "1")) (class (assoc-ref json "class"))
			 (dummy (Show! "2")) (key (assoc-ref json "key"))
			 (dummy (Show! "3")) (value (assoc-ref json "value"))
			 (dummy (Show! "4 " username " " class " " key " " value)) (answer (mtfa-bc::AddKV username class key value)))
		    (Show! "Torna: " answer)
	 ...
	 (receive (authorized username) (CheckJwtToken pbuf cfg::FUNUF::secret "DelKV")
	     (ifnot authorized
		    ...
		    (let* ((class (assoc-ref json "class"))
			   ...
			   (answer (mtfa-bc::DelKV username class key))
			   )
		      (JSON-ANSWER::success "Ok" `("answer" . ,answer)))))))
;;
  ...
  (ifnot (ConnectToTheBlockchain)
	 ...
	 (receive (authorized username) (CheckJwtToken pbuf cfg::FUNUF::secret "GetClasses")
	     (ifnot authorized
		    ...
		    (JSON-ANSWER::success "Ok" `("answer" . ,(mtfa-bc::GetClasses username)))))))
;;
	 ...
	 (receive (authorized username) (CheckJwtToken pbuf cfg::FUNUF::secret "GetNumKeys")
	     (ifnot authorized
		    ...
		    (let* ((class (assoc-ref json "class"))
			   ...
			   (answer (mtfa-bc::GetNumKeys username class key)))
		      (JSON-ANSWER::success "Ok" `("answer" . ,answer)))))))
;;
	 ...
	 (receive (authorized username) (CheckJwtToken pbuf cfg::FUNUF::secret "GetTxData")
	     (ifnot authorized
		    ...
		    (let ((txid (assoc-ref json "txid")))
		      (JSON-ANSWER::success "Ok" `("answer" . ,(mtfa-bc::GetTxData username txid cfg::channel-to-connect))))))))
...
(define (GestioneAPIBackEnd json-body pbuf actionl auxiliary-info)
  ...
  (let* ((json-body (json-string->scm json-body)) ;;Converte in json
	...
	(let ((dummy (Show! "Call: " fun json-body))
	      (ans (fun json-body pbuf actionl auxiliary-info)))
	  ans))))
...
(Show! "Connected!")
;;(Show! "Add: " (mtfa-bc::AddKV "franco" "classe" "chiave" "valore"))
