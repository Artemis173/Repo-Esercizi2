...
async function getCurrentGasMediumPrice(network) {
  let gasLimit = 6721975; //web3.utils.toHex(2100000000);
  let gasPrice = 20000000000; //web3.utils.toHex(web3.utils.toWei("1", "gwei"));
...
if (
    ...
    let gasLimit = web3.utils.toHex(100000000);
  ...
  try {
    switch (
      ...
      case "GiveProduct":
        {
          if (request.length != 5) {
            return { result: false, code: "GiveProduct produttore distributore prodotto quantita" };
          }
          ...
          try {
            await contract.methods.ProduttoreDistributore(
              request[1],
              request[2],
              request[3],
              parseInt(request[4])
            ).send(tx);
            return { result: true, data: { ProduttoreDistributore: true } };
          } catch (err) {
            console.log(err);
            return { result: false, code: "error in ProduttoreDistributore" };
          }
        }
        break;
        case "GetProducts":
          {
            if (request.length != 2) {
              return { result: false, code: "GetProducts produttore" };
            }
            let tokens = await contract.methods.GetProdottiFromDistributore(request[1]).call(tx);
            console.log(tokens);
            return { result: true, data: { GetProducts: tokens } };
          }
          break;
        case "addone":
        {
